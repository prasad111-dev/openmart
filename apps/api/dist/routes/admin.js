...
router.post('/shops', authenticate, authorize('ADMIN'), async (req, res) => {
    try {
...
router.put('/shops/:id/approve', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.put('/shops/:id/reject', authenticate, authorize('ADMIN'), async (req, res) => {
    const shop = await prisma.shop.findUnique({
        where: { id: req.params.id },
        include: { owner: true },
    });
...
router.put('/shops/:id/toggle', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.get('/delivery-boys', authenticate, authorize('ADMIN'), async (_req, res) => {
...
router.get('/seller-settlements', authenticate, authorize('ADMIN'), async (_req, res) => {
    try {
        const shops = await prisma.shop.findMany({
            where: { isApproved: true },
            include: {
        ...
        const settlements = shops.map((shop) => {
            const totalSales = shop.orders.reduce((sum, o) => sum + Number(o.totalAmount), 0);
...
router.get('/seller-settlements/:shopId', authenticate, authorize('ADMIN'), async (req, res) => {
    try {
...
router.post('/seller-payments', authenticate, authorize('ADMIN'), async (req, res) => {
    try {
...
router.get('/seller-alerts', authenticate, authorize('ADMIN'), async (_req, res) => {
    try {
        const shops = await prisma.shop.findMany({
            where: { isApproved: true },
            include: {
...
router.put('/platform-settings', authenticate, authorize('ADMIN'), async (req, res) => {
    try {
        const { platformFee, deliveryCharge, freeDeliveryThreshold, minimumOrderAmount, basicPlanPrice, basicPlanFeatures, standardPlanPrice, standardPlanFeatures, premiumPlanPrice, premiumPlanFeatures, defaultCommissionRate } = req.body;
...
router.put('/delivery-boys/:id', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.put('/shops/:id', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.put('/orders/:id/force-cancel', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.post('/orders/:id/dispute', authenticate, authorize('ADMIN'), async (req, res) => {
