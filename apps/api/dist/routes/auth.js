...
router.post('/admin/register-user', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.put('/admin/users/:id/block', authenticate, async (req, res) => {
    if (req.user?.role !== 'ADMIN') {
        return res.status(403).json({ success: false, error: 'Access denied' });
    }
...
router.get('/admin/users/:id/addresses', authenticate, async (req, res) => {
    if (req.user?.role !== 'ADMIN') {
        return res.status(403).json({ success: false, error: 'Access denied' });
    }
