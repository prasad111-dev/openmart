...
router.put('/:id/status', authenticate, authorize('SHOP_OWNER', 'ADMIN'), async (req, res) => {
...
router.post('/:id/assign-delivery', authenticate, authorize('SHOP_OWNER'), async (req, res) => {
...
router.put('/:id/deliver', authenticate, authorize('DELIVERY_BOY', 'ADMIN'), async (req, res) => {
    const order = await prisma.order.findUnique({ where: { id: req.params.id } });
    if (!order) {
        return res.status(404).json({ success: false, error: 'Order not found' });
    }
