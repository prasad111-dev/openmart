...
router.get('/', async (req, res) => {
    ...
    products = products.filter((p) => p.shop && p.shop.isOpen);
    if (shopId) {
        products = products.filter((p) => p.shopId === String(shopId));
    }
    if (categoryStr) {
        products = products.filter((p) => p.category.toLowerCase() === categoryStr);
    }
    if (searchStr) {
    ...
    if (pincode) {
        products = products.filter((p) => p.shop.pincode === String(pincode));
    }
