...
router.get('/', authenticate, async (req: AuthRequest, res) => {
  const userId = req.user?.userId
   ...
   const cartItems = await prisma.cartItem.findMany({
    ...
    const shopIds = [...new Set(cartItems.map((item: { product: { shopId: string | null } }) => item.product?.shopId).filter((id: string | undefined): id is string => id !== null && id !== undefined))]
   ...
   const shopMap = Object.fromEntries(shops.map((s: { id: string }) => [s.id, s]))
