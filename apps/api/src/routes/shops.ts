...
router.get('/', async (req, res) => {
  ...
  if (pincode) {
    shops = shops.filter((s: any) => s.pincode === String(pincode))
  }
  
  if (categoryStr) {
    shops = shops.filter((s: any) => s.category.toLowerCase() === categoryStr)
  }
  ...
  const shopsWithImages = shops.map((shop: any) => ({
    ...shop,
...
router.get('/categories', async (_req, res) => {
  ...
  res.json({ success: true, data: categories.map((c: { name: string }) => c.name) })
})
...
router.post('/', authenticate, authorize('SHOP_OWNER'), async (req: AuthRequest, res) => {
...
router.put('/:id', authenticate, async (req: AuthRequest, res) => {
  ...
  if (deliveryRadius !== undefined) updateData.deliveryRadius = parseInt(deliveryRadius) || deliveryRadius
...
router.post('/:id/products', authenticate, authorize('SHOP_OWNER'), async (req: AuthRequest, res) => {
  const shop = await prisma.shop.findUnique({ where: { id: req.params.id } })
  if (!shop) {
    return res.status(404).json({ success: false, error: 'Shop not found' })
  }
...
router.put('/:id/products/:productId', authenticate, authorize('SHOP_OWNER'), async (req: AuthRequest, res) => {
...
router.delete('/:id/products/:productId', authenticate, authorize('SHOP_OWNER'), async (req: AuthRequest, res) => {
  const product = await prisma.product.findFirst({
    where: { id: req.params.productId, shopId: req.params.id },
  })
  if (!product) {
...
router.get('/:id/orders', authenticate, authorize('SHOP_OWNER', 'ADMIN'), async (req: AuthRequest, res) => {
  const shop = await prisma.shop.findUnique({ where: { id: req.params.id } })
...
router.put('/:id/toggle-open', authenticate, authorize('SHOP_OWNER'), async (req: AuthRequest, res) => {
  const shop = await prisma.shop.findUnique({ where: { id: req.params.id } })
...
router.get('/:id/delivery-boys', authenticate, async (req: AuthRequest, res) => {
  const shop = await prisma.shop.findUnique({ where: { id: req.params.id } })
  if (!shop) {
    return res.status(404).json({ success: false, error: 'Shop not found' })
  }
